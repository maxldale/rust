...
pub fn palindromify(lst: &mut Vec<i32>) -> () {
...
pub fn rotate<'a>(lst: &'a [i32]) -> Vec<i32> {
...
pub fn get_match<'a>(words: &'a str, match_word: &'a str) -> Option<&'a str> {
...
pub fn squared_cubed_sum(lst: Vec<i32>) -> i32 {
    let owned_lst = lst.to_owned();
    let sq = squared_sum(lst);
    let cube = cubed_sum(owned_lst);
    sq + cube
}
